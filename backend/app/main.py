...
# F3-03: curated OpenAPI metadata. tags_metadata groups operations in the
...
# F3-03: external documentation pointer surfaced in the OpenAPI top-level
...
# F3-03: list of public server URLs surfaced under ``servers``. Local is
...
def _custom_openapi() -> dict:
    """F3-03: attach the curated OpenAPI ``externalDocs`` pointer to the
...
async def health(request: Request, response: Response) -> dict[str, str]:
    # F3-06: static discovery Link headers so uptime monitors and agents can
...
# F3-06: serve the public API catalog linkset at the conventional
...
async def api_catalog() -> JSONResponse:
    # F3-06: serve the static linkset with the RFC 9264 media type so
...
# F3-02: explicit error mapping so the autogenerated OpenAPI schema documents
# the exact public error shapes for agents.
# F3-03: each entry now also carries an ``examples`` payload so agents can
